...
def parse_with_xpath(url_list, bloom):
    ...
    # print(country_urls)
    # 获取国家URL
    urls_country = get_country_urls(url_list, empories_url)
...
def thread_start(continent_url):
    bloom = get_building_url()
    html = get_one_page(continent_url)
    etree_html = etree.HTML(html)
    country = etree_html.xpath('//ul[@class="list noicon-list two-columns"]')
    if country:
        if len(country) > 2:
            country_urls_list1 = country[0].xpath('li/a/@href')
            country_urls_list2 = country[1].xpath('li/a/@href')
            country_urls_list = country_urls_list1 + country_urls_list2
        else:
            country_urls_list = country[0].xpath('li/a/@href')
        country_urls_list = [country_urls_list[i:i+2] for i in range(0, len(country_urls_list), 2)]
        print(country_urls_list, len(country_urls_list))
        # country_urls_list =  ['/country/100008/argentina',
        #                        '/country/100021/bolivia',
        #                     '/country/100024/brazil',
        #                     '/country/100029/chile',
        #                      '/country/100129/paraguay',
        #                      '/country/100179/uruguay',
        #                      '/country/100183/venezuela']
        # country_urls_list = [country_urls_list[i:i + 1] for i in range(0, len(country_urls_list), 1)]
        # print(country_urls_list, len(country_urls_list))
        # # # 线程组
        threads = []
        for urls_list in country_urls_list:
            # print(len(urls))
            t = Thread(target=emporis_start, args=(urls_list, bloom))
            t.start()
            threads.append(t)
        # 先执行完所有线程再执行主进程
        for t in threads:
...
def emporis_start(url, bloom):
    items = parse_with_xpath(url, bloom)
    num = write_db(items)
    # continent_name = continent.split('/')[-1]
    print(f'{url}的数据已经爬取完毕！！数据库更新了'+str(num)+'条')


if __name__ == '__main__':
    continent_url = 'https://www.emporis.com/continent/100001/europe'
    thread_start(continent_url)
