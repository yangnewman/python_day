...
class transfer:
    ...
    def gcj02_to_wg84(self,gcj02_lng,gcj02_lat):
        ...
        wg84_Lng, wg84_Lat = self.cut_num_6(wg84_Lng,wg84_Lat)
    ...
    def cut_num_6(self, lng, lat):
        # 保留6位小数
        lng_s = str(lng)
        lat_s = str(lat)
        lng_list = lng_s.split('.')
        lat_list = lat_s.split('.')
        lng = float(lng_list[0] + '.' + lng_list[-1][:6])
        lat = float(lat_list[0] + '.' + lat_list[-1][:6])
        return lng, lat


if __name__ == '__main__':
    tr = transfer()
    print(tr.bd09_to_wg84(103.267563, 36.740055))
